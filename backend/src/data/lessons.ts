export type Slide =
  | { type: 'intro' | 'content' | 'story'; content: { image: string; text?: string; story?: string; lesson?: string } }
  | {
      type: 'question';
      content: {
        question: string;
  ...
  | { type: 'completion'; content: { message: string; xp: number; badge?: string | null } };
...
const specificLessons: Record<string, LessonContent> = {
  '1.1': {
    title: 'What is Money?',
    slides: [
...
export const getLessonContent = (moduleId: string, lessonId: string): LessonContent => {
